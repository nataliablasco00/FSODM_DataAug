...
# import polyiou
"""
    some basic functions which are useful for process data
"""
...
def parse_dior_poly(filename):
    ...
    for obj in root.findall('object'):
        object_struct = {}
        object_struct['name'] = obj.find('name').text
        ...
        object_struct['poly'] = [(xmin, ymin), (xmax, ymin), (xmax, ymax), (xmin, ymax)]
    ...
    if filename != '':
        ...
        while True:
            ...
            if line:
                ...
                object_struct['name'] = int(splitlines[4])
                object_struct['difficult'] = '0'
                x1 = float(splitlines[0].strip('('))
                y1 = float(splitlines[1].strip(')'))
                x2 = float(splitlines[2].strip('('))
                y2 = float(splitlines[3].strip(')'))
                object_struct['poly'] = [(x1, y1), (x2, y1), (x2, y2), (x1, y2)]
                gtpoly = shgeo.Polygon(object_struct['poly'])
                object_struct['area'] = gtpoly.area
                objects.append(object_struct)
            else:
