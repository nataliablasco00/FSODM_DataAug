...
def valid(datacfg, darknetcfg, learnetcfg, weightfile, outfile, use_baserw=False):
    ...
    for batch_idx, (data, target) in enumerate(valid_loader):
        ...
        # import pdb; pdb.set_trace()
        batch_boxes = get_region_boxes_v4(output, n_cls, conf_thresh, m.num_classes, m.anchors, m.num_anchors,\
                                          m.loss.feature_scale, (m.height, m.width), 1, 1)

        # for i in range(len(batch_boxes)):
        #     assert len(batch_boxes[i]) > 0, 'batch_boxes[{}] == 0'.format(i)
        ...
        for b in range(bs):
            ...
            for i in range(n_cls):
                nms_boxes.extend(nms([x for x in boxes if x[6] == i], nms_thresh))
